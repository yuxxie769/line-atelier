...
`paint_reference_pass` 是可选的算法辅助工具。模型决定区域、步骤和参数，工具从参考图取色并提取轨迹；不要把这种方式描述为模型凭空逐点构思整幅画。

```json
{"mode":"color","region":[0,0,586,200],"spacing":1.5,"tolerance":12,"stage":"detail","layer":"paper","includePaper":true,"animate":true}
```

- `mode: "outline"` 生成连续轮廓指引；`mode: "color"` 生成蛇形扫描线笔迹。
- `region` 是画布像素坐标 `[x,y,width,height]`，省略则使用全图。
- `spacing` 为 1–24 像素。建议先用 5–8 铺底，再用 1–2 分区补细节。
- `tolerance` 为相邻色彩合并阈值，0–80；越小越接近参考图，也会生成更多笔迹。
- `includePaper: true` 同时绘制白色区域，清理前面粗笔刷溢出的边缘。
- `dryRun: true` 仅返回预计笔数。单批超过 10,000 笔会在修改画布之前拒绝，应拆分区域。
- 所有结果都走同一套笔迹校验和 Canvas 笔刷执行，不调用生图服务，不向作品画布贴参考图片。

模型面板中的“已收到模型工具调用”只会在实际执行 WebMCP 工具时出现，并显示本页面会话的调用次数。页面重载会清空这个计数；“浏览器工具已注册”仅表示浏览器支持注册，不表示已被模型操作。

“载入本次角色临摹”打开已保存的 46,002 笔工程。它是一次真实 WebMCP 操作的结果回放，不会重新启动模型。
