...
返回内容：对应参考局部、当前画布局部、隐藏底稿的独立图和叠图、标出范围的整幅图；笔迹名称、稳定 ID、图层、意图、原始几何、解析后的整图坐标、局部坐标、物体归一化坐标和相关锚点。图片中的底稿编号对应 `commands[].label`。贝塞尔控制点明确标为 `bezier-control`，不能当成曲线上经过点；旧采样轨迹默认最多取 32 点，完整点可用 `paint_get_strokes({ids:[...]})` 读取。

候选按显式指定、目标底稿、祖先物体底稿、空间相交底稿、其他笔迹排序。例如人体层的眼线即使归属 figure，也能随左眼查询返回。模型需要自己判断这些线是否有用、是否准确；空间相邻不等于语义正确。`guideIds` 可以显式补充候选。`nextOffset` 非空时用相同参数和返回的 offset 读取下一页（每页默认 80 条）；每页图像仅标注该页底稿。
...
使用习惯：先看图，再读相关线的坐标，自行决定沿用、偏移或修订底稿；小批落笔后复看。此工具不建立自动追随底稿的关系，不把候选线变为强制约束。界面入口：选择当前物体 → “查看底稿与定位依据”。
