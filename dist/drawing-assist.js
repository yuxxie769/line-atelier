...
export function createDrawingAssist(engine,{save,record,toast,review,objectId,brush}) {
  ...
  function updateStrokeOptions(){const id=el('pressure-stroke').value,object=objectId(),rows=engine.doc.commands.filter(c=>c.type==='stroke'&&(!object||object==='figure'||c.objectId===object)).sort((a,b)=>(a.subphase==='clean'?0:1)-(b.subphase==='clean'?0:1));el('pressure-stroke').replaceChildren(...rows.map(c=>new Option((c.part||c.subphase)+' · '+c.id,c.id)));if(rows.some(c=>c.id===id))el('pressure-stroke').value=id;el('smooth-stroke').disabled=el('pressure-apply').disabled=el('width-apply').disabled=!rows.length;}
  ...
  el('stroke-smoothing').oninput=e=>{el('stroke-smoothing-value').textContent=e.target.value+'%';};
  el('smooth-stroke').onclick=safeTask(()=>{const c=selectedStroke();if(!c)throw Error('请选择笔迹');const result=engine.smoothStrokes({ids:[c.id],smoothing:Number(el('stroke-smoothing').value)/100});if(result.changed){save();record('平滑所选笔迹');}el('pressure-status').textContent='已应用平滑，可撤销；设为 0 恢复原线。';});
