...
当前内置工程：20,972 笔，6 个图层，7 个阶段。新增矢量选区内的连续笔刷铺色、正片叠底/滤色、裁切图层、整笔透明度和 paint_material_pass。完整方法与证据见 dist/method-notes.md；模型调用说明见 dist/agent-guide.md。

手工编排形体方向和绘画顺序，参考图算法生成色块与大量笔迹。没有外部生图服务或模型 API Key。所有可见笔迹可导出和回放。
