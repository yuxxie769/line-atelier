...
const doc=JSON.parse(await read('dist/line-atelier-v4-r2.line.json'));
...
for(const file of ['geometry.js','pressure.js','model.js','renderer.js','reference.js','document-export.js','connections.js','diagnostics.js','engine.js','drawing-assist.js','app.js']){
  script+=(await read('dist/'+file)).replace(/^import[^\n]*\n/gm,'').replace(/^export /gm,'')+'\n';
}
script=script.replaceAll("fetch('./line-atelier-v4-r2.line.json')","Promise.resolve({ok:true,json:async()=>structuredClone(EMBEDDED_STUDY)})")
  .replaceAll("'./reference.png'",'EMBEDDED_REFERENCE')
  .replaceAll("'line-atelier-v4-r2'","'line-atelier-v4-r2-standalone'");
...
let html=(await read('dist/index.html'))
  ...
  .replace('href="./line-atelier-v4-r2.html"','href=""')
...
for(const folder of ['downloads','dist'])await writeFile(new URL(folder+'/line-atelier-v4-r2.html',root),html);
