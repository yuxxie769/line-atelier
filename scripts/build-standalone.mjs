...
const doc=JSON.parse(await read('dist/body-first-study.line.json'));
...
for(const file of ['geometry.js','model.js','renderer.js','reference.js','document-export.js','engine.js','app.js'])script+=(await read('dist/'+file)).replace(/^import[^\n]*\n/gm,'').replace(/^export /gm,'')+'\n';
script=script.replaceAll("fetch('./body-first-study.line.json')","Promise.resolve({ok:true,json:async()=>structuredClone(EMBEDDED_STUDY)})").replaceAll("'./reference.png'",'EMBEDDED_REFERENCE');
...
html=html.replace('href="./line-atelier-v4-body-first-full-lineart.html"','href=""');
await mkdir(new URL('downloads/',root),{recursive:true});
await writeFile(new URL('downloads/line-atelier-v4-body-first-full-lineart.html',root),html);
await writeFile(new URL('dist/line-atelier-v4-body-first-full-lineart.html',root),html);
