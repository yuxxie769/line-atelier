...
for(const file of ['geometry.js','model.js','renderer.js','reference.js','document-export.js','connections.js','engine.js','app.js'])script+=(await read('dist/'+file)).replace(/^import[^\n]*\n/gm,'').replace(/^export /gm,'')+'\n';
...
html=html.replace('href="./line-atelier-v4-structure-repair.html"','href=""').replace('href="./structure-comparison.html"','href="https://line-atelier.yuxxie.chatgpt.site/structure-comparison.html"');
await mkdir(new URL('downloads/',root),{recursive:true});
await writeFile(new URL('downloads/line-atelier-v4-structure-repair.html',root),html);
await writeFile(new URL('dist/line-atelier-v4-structure-repair.html',root),html);
